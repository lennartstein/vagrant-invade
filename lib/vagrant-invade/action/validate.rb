      ...
      class Validate
        ...
        def call(env)
          ...
          machines.each_with_index do |(machine, section), index|
            ...
            # Box
            @env[:ui].info("[Invade] Machine #{machine.upcase} - Box") if @invade['verbose']
            ...
            # Network
            @env[:ui].info("[Invade] Machine #{machine.upcase} - Network") if @invade['verbose']
            ...
            # Virtual machine
            @env[:ui].info("[Invade] Machine #{machine.upcase} - VM") if @invade['verbose']
            ...
            @env[:ui].info("[Invade] Machine #{machine.upcase} - Synced Folders") if @invade['verbose']
            section['synced_folder'].each_with_index do |(sf, option), index|
              @env[:ui].info("\t#{sf.upcase}:") if @invade['verbose']
            ...
            # PUPPET
            @env[:ui].info("[Invade] Machine #{machine.upcase} - Puppet") if @invade['verbose']
            ...
            # SSH
            @env[:ui].info("[Invade] Machine #{machine.upcase} - SSH") if @invade['verbose']
            ...
            # PLUGINS
            @env[:ui].info("[Invade] Machine #{machine.upcase} - Plugins") if @invade['verbose']

            # PLUGIN: Hostmanager
            @env[:ui].info("\tHostmanager:") if @invade['verbose']
            ...
            # PLUGIN: Winnfsd
            @env[:ui].info("\tWinnfsd:") if @invade['verbose']
        ...
        def validate_boolean(value, name, default)
          ...
          if [true, false].include? value
            @env[:ui].success("\t#{name} => #{value}") if @invade['verbose']
          else
            @env[:ui].warn("\t#{name} => #{value} could not be validated - set to #{default}.") if value === ''
            return default
          end
        ...
        def validate_integer(value, name, default)
          ...
          if value.is_a? Integer || value.is_numeric?
            @env[:ui].success("\t#{name} => #{value}") if @invade['verbose']
          else
            @env[:ui].warn("\t#{name} => #{value} could not be validated - set to #{default}.") if value === ''
            return default
